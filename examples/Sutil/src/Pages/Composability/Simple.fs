module Examples.Sutil.Pages.Composability.Simple
...
type State =
    | Filling of Form.View.Model<Composability.Simple.Values>
...
let Page () =
    let stateStore = Composability.Simple.init |> State.Filling |> Store.make
        ...
        fun state ->
            ...
            | State.Filled(name, address) ->
                renderSubmittedView
                    name
                    address
                    (fun _ -> Composability.Simple.init |> State.Filling |> Store.set stateStore)
    )
